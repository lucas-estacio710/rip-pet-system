...
export default function RepassesPage() {
  ...
  // Visibilidade por FLS (`tela_repasses`): a tela é da Matriz. As demais unidades
  // recebem seed `hidden` na migration 107. super_admin sempre enxerga.
  const { hasModule } = useUnit()
  const podeVer = hasModule('tela_repasses')
  ...
  const travado = existente?.status === 'pago'
  ...
  if (!podeVer) {
    return <EmptyState icon={Shield} title="Acesso restrito" description="O repasse é gerido pela Matriz." />
  }
  ...
  return (
    <div className="animate-fade-in space-y-2">
      ...
      <div className="flex flex-wrap items-center gap-3">
        ...
        <h1 className="text-title text-[var(--shell-text)]">Repasse</h1>
        <input
            ...
            {existente && (
              <span className="flex items-center gap-1.5 text-xs text-[var(--surface-500)]">
                ...
                <select
                  ...
                  className="input text-xs w-24 py-0.5 ml-1"
