...
function LoginForm() {
  ...
  // Mostrar erro do callback de autenticação
  useEffect(() => {
    const callbackError = searchParams.get('error')
    if (callbackError === 'auth_callback_failed') {
      setError('Link de autenticação expirado ou inválido. Tente novamente.')
    }
  }, [searchParams])
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-slate-900 via-slate-800 to-purple-900">
      <div className="w-full max-w-md px-4">
        <Suspense fallback={
          <div className="bg-slate-800 rounded-2xl shadow-2xl p-8 flex justify-center">
